...
type Trigger = 'mount' | 'hover' | 'scroll' | 'loop' | 'hold';
...
export function FoldPiece({
  ...
  useEffect(() => {
    ...
    const tween = gsap.fromTo(pieces, fromVars, {
      ...toVars,
      repeat: trigger === "loop" ? -1 : 0,
      yoyo: trigger === "loop",
      repeatDelay: trigger === "loop" ? 0.55 : 0,
    });
    return () => {
      tween.kill();
      gsap.killTweensOf(pieces);
    };
  }, [hinge, duration, ease, safeCrease, trigger, hingeConfig.origin, hingeConfig.rotateX, hingeConfig.rotateY]);
